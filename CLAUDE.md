...
3. **Always resolve permissions through the shared resolver, at the SQL level.** Use the auth helpers (`requireSession` / `requireWorkspaceMember` / `requirePagePermission`); never filter restricted rows in application code after a broad fetch (that is a BOLA vulnerability — Phase-1 decision #5). A private page (`is_private = true`) short-circuits inheritance: only the creator + explicit grants apply, workspace Admins included in the denial.
4. **Page hierarchy is a closure table.** Maintain `page_closure` in the **same transaction** as any `parent_id` change (create, move, delete). Never compute descendants by recursive app-side queries (Phase-1 decision #1).
